/* Loans & Receivables tab. Per §8.3 we have three sections:
   ...
   - Receivables  (management and settlement view only — no entry form here;
                   log a receivable via Transactions tab with type='receivable') */
(function () {
  "use strict";
  ...
  async function render(container) {
    ...
    container.innerHTML = `
      ...
      <div class="neo-card">
        ...
        <p class="muted" style="margin:0 0 12px;font-size:0.85rem">
          ⟳ To log a new receivable, go to the
          <a href="#/transactions" id="link-to-tx" style="cursor:pointer;text-decoration:underline">Transactions tab</a>
          and choose type <strong>Receivable (reimbursable spend)</strong>.
        </p>
    ...
    document.querySelectorAll("[data-rec]").forEach(b => {
      b.addEventListener("click", () => openReceivableActions(parseInt(b.dataset.rec, 10), recR.data.receivables.find(r => r.id === parseInt(b.dataset.rec, 10)), () => render(container)));
    });

    document.getElementById("link-to-tx")?.addEventListener("click", (e) => {
      e.preventDefault();
  ...
  function renderReceivableList(items) {
    if (!items.length) return `<p class="muted">No receivables logged yet. Use the Transactions tab to log one.</p>`;
  ...
  async function openReceivableActions(rid, recData, onChange) {
    // recData is the receivable object from the list (has transaction_id if new-style)
    const isLegacy = !recData?.transaction_id;
    const today = new Date().toISOString().slice(0,10);

    const m = window.U.modal(`
      <h2>Manage Receivable</h2>
      <p class="muted">${escapeHtml(recData?.description || "")} — ${fmtAED(recData?.amount || 0)}</p>
      <p class="muted" style="font-size:0.85rem">Month: ${escapeHtml(recData?.month || "")} • Logged: ${fmtDate(recData?.date_logged || "")}</p>

      <h3 style="margin:14px 0 8px">Settlement date</h3>
      <input class="neo-input" type="date" id="settle-date" value="${today}" style="margin-bottom:16px;width:100%">

      <div class="form-actions" style="flex-direction:column;align-items:stretch;gap:8px">
        <button class="neo-btn neo-btn-primary" id="btn-settle-bank">✓ Settled — money received into Bank</button>
        <button class="neo-btn neo-btn-primary" id="btn-settle-petty">✓ Settled — money received into Petty Cash</button>
        <button class="neo-btn neo-btn-danger" id="btn-convert">⚠ Convert to expense (won't be reimbursed)</button>
        ${isLegacy ? `<button class="neo-btn" id="btn-del">Delete record (legacy)</button>` : ""}
        <button class="neo-btn" id="btn-cancel">Cancel</button>
      </div>
      <p class="form-error" id="rec-act-err"></p>`);
    ...
    async function doSettle(dest) {
      ...
      const r = await api(`/api/receivables/${rid}/settle`, { method: "POST", body: { destination: dest, date }});
      if (!r.ok) {
        document.getElementById("rec-act-err").textContent = r.data?.error || "Failed";
        return;
      }
      m.close();
      toast(`Settled into ${dest === "bank" ? "Bank" : "Petty Cash"} ✓`);
      if (onChange) onChange();
    }

    async function doConvert() {
      if (!confirm(
        "Converting reclassifies this as a real expense in the month it was originally logged.\n\n" +
        "• It will now count toward that month's budget.\n" +
        "• If that month is already closed, the cascade will be replayed forward.\n\n" +
        "Continue?"
      )) return;
      ...
      let msg = "Converted to expense";
      if (r.data.replayed_months?.length) {
        msg += `; cascade replayed: ${r.data.replayed_months.join(", ")}`;
      }
      if (r.data.pot_went_negative) {
        msg += " ⚠ Savings pot went negative — check Audit Log";
      }
      m.close();
      toast(msg, r.data.pot_went_negative ? "error" : undefined);
      if (onChange) onChange();
    }

    async function doDelete() {
      if (!confirm("Delete this receivable record? Only legacy records (not linked to a transaction) can be deleted here.")) return;
